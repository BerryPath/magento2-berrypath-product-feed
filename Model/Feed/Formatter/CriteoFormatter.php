...
class CriteoFormatter implements ChannelFormatterInterface
{
    ...
    public function toXml(array $feed, array $options = []): string
    {
        return $this->writer->rowsToXml(
            $this->rowBuilder->criteoRows($feed, $options),
            'products',
            'product',
    ...
    public function toJson(array $feed, array $options = []): string
    {
        $json = json_encode(
            [
                ...
                'items' => $this->rowBuilder->criteoRows($feed, $options),
            ],
    ...
    {
        return $this->writer->rowsToCsv($this->rowBuilder->criteoRows($feed, $options));
    }
}
